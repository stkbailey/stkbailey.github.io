def submit_brain_for_printing(
    subj_id,
    scratch_dir="/scratch/bailesk1",
    image_path=None,
    decimation_level=0.15,
    anat_scan_type=None,
):
...
cp $DLPATH {ip}""".format(
            sid=subj_id, st=anat_scan_type, scr=scratch_dir, ip=image_path
        )
...
""".format(
        ...
        sid=subj_id,
        ip=image_path,
        d="{}".format(decimation_level),
        dp="{}".format(int(decimation_level * 100)),
    )
    ...
    {script}
    """.format(
        fs=fs_subj_dir, sid=subj_id, script=fs_cmd
    )
