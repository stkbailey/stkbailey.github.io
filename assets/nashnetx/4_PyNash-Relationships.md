...
```
    ...
    <ipython-input-13-c0c558bdffa8> in <module>()
          3 node_order = df_members.sort_values(by='centrality').index
          4 adjmat_weighted = nx.to_numpy_array(gm, nodelist=node_order)
    ----> 5 adjmat_binary = weighted_adjmat > 0
          6 
          7 ax.imshow(adjmat_binary, cmap='hot')
    

    NameError: name 'weighted_adjmat' is not defined
